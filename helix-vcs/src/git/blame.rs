...
impl FileBlame {
    ...
    fn blame_for_line_inserted_removed(
        ...
        let author_time = author.as_ref().and_then(|a| a.time().ok());
        ...
        LineBlame {
            ...
            time_ago: author_time
                .map(|time| helix_stdx::time::format_relative_time(time.seconds, time.offset)),
        }
    }
    ...
    pub fn try_new(file: PathBuf) -> Result<Self> {
        ...
        let workdir = repo.workdir().context("Could not get repo worktree")?;
        let relative_path = file.strip_prefix(workdir)?;
        let relative_path = gix::path::to_unix_separators_on_windows(gix::path::try_into_bstr(
            relative_path,
        )?);
        let file_blame = repo
            .blame_file(relative_path.as_ref(), head, Default::default())?
            .entries;
